...
import { planKeyboard, planMessage, planSessions, refreshPlanUi, type PlanSession } from "./plan_session";
import { generatePlan } from "../plan/planner";
import { approvalDiff, approvalSessions } from "./approval_session";
...
export  function registerHandler(bot:Telegraf) {
    ...
    bot.command("ask" , async (ctx)=>{
        ...
        if(!q)
             return ctx.reply("Usage: `/ask <ur question>`" , {parse_mode:"Markdown"})
